...
describe("attendanceHelpers", () => {
  it("extracts checkout window times from backend messages", () => {
    expect(
      getCheckoutWindowSummary("Đã hết hạn tan ca (Vào làm: 08:35; Tan ca: 17:00 (hạn chót 20:00))")
    ).toEqual({
      checkInTime: "08:35",
  ...
  it("summarizes expired checkout reasons without parenthetical noise", () => {
    expect(
      getAttendanceIssueSummary("Đã hết hạn tan ca (Vào làm: 08:35; Tan ca: 17:00 (hạn chót 20:00))")
    ).toEqual({
