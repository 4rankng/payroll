...
interface EmployeeAdSheetProps {
  ...
  onClose: () => void;
  banner: AdBanner;
  /** Fired before any CTA action; the caller owns click recording + navigation. */
  onCTAClick?: (cta: AdBannerCTA, index: number) => void;
}

/**
 * Full-campaign bottom sheet. Pure presentational so the admin composer can
 * render the exact same markup in its 375px live preview — there is no second
 * copy of the campaign layout to drift.
 */
