...
interface ResetEmployeePasswordModalProps {
  open: boolean;
  onClose: () => void;
  onResetPassword: (employeeId: number, password: string) => void;
...
export function ResetEmployeePasswordModal({
  ...
  employee,
  loading = false
}: ResetEmployeePasswordModalProps) {
  ...
  const handleSubmit = () => {
    ...
    onResetPassword(employee.id, password);
    handleClose();
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Đặt lại mật khẩu cho nhân viên <strong>{employee?.fullname}</strong>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="bg-muted/50 rounded-xl p-3 space-y-2">
            ...
            <p className="typography-body-medium text-muted-foreground mt-2">
              Nhân viên sẽ phải sử dụng mật khẩu mới này để đăng nhập.
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 h-8 px-3 rounded bg-amber-500 text-white text-sm font-medium whitespace-nowrap hover:bg-amber-600 transition-colors disabled:opacity-50 disabled:pointer-events-none"
          >
