  ...
  if (showResults && importResult) {
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg p-0 gap-0 overflow-hidden" hideCloseButton>
          <DialogNavyHeader
            title="Kết quả nhập"
                ...
                <ScrollArea className="h-44 rounded-xl border">
                  <div className="p-3 space-y-2">
                    {importResult.errors?.map((error, index) => (
                      <div key={index} className="flex items-start gap-2.5 p-2.5 rounded-xl bg-destructive/5 border border-destructive/10">
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 gap-0 overflow-hidden" hideCloseButton>
        ...
        <DialogFooter className="px-5 py-4 border-t flex flex-row gap-2">
          <Button variant="outline" onClick={handleClose} disabled={importMutation.isPending} className="min-w-[72px]">
            Đóng
          </Button>
