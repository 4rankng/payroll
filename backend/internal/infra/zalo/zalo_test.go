...
// --- provider.go: ExchangeCode ---------------------------------------------

func TestProvider_ExchangeCode_Happy(t *testing.T) {
	p, _, creds, _ := newProviderWithMock(t)
	// Reset to pre-OAuth state (no tokens) to simulate the connect flow.
	creds.mu.Lock()
	creds.cur.AccessToken = ""
	creds.cur.RefreshToken = ""
	creds.cur.ExpiresAt = nil
	creds.mu.Unlock()

	out, err := p.ExchangeCode(context.Background(), "authcode-123", "")
	if err != nil {
...
func TestProvider_ExchangeCode_NoClobberOnEmptyRefresh(t *testing.T) {
	p, mock, creds, _ := newProviderWithMock(t)
	// Pre-existing refresh_token that must NOT be clobbered if Zalo omits one.
	creds.mu.Lock()
	creds.cur.RefreshToken = "keep-me"
	creds.mu.Unlock()
	// OAuth returns access but no refresh.
	mock.mu.Lock()
	mock.oauthResp = `{"access_token":"a1","expires_in":3600}`
	mock.mu.Unlock()

	out, err := p.ExchangeCode(context.Background(), "code", "")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if out.RefreshToken != "keep-me" {
		t.Errorf("RefreshToken clobbered to %q, want keep-me", out.RefreshToken)
	}
}

func TestProvider_ExchangeCode_NoAccessTokenReturnsError(t *testing.T) {
	p, mock, _, _ := newProviderWithMock(t)
	mock.mu.Lock()
	mock.oauthResp = `{"access_token":"","refresh_token":"r","expires_in":3600}`
	mock.mu.Unlock()
	_, err := p.ExchangeCode(context.Background(), "code", "")
	if err == nil {
		t.Fatal("expected error when OAuth returns no access_token")
	}
}

func TestProvider_ExchangeCode_NotConfiguredWithoutAppID(t *testing.T) {
	creds := &fakeCreds{cur: Credentials{AccessToken: "x"}} // no AppID/Secret
	p := NewProvider(creds, Config{SendURL: "http://x", OAuthURL: "http://y", HTTPTimeout: time.Second}, nil)
	_, err := p.ExchangeCode(context.Background(), "code", "")
	if err != ErrNotConfigured {
		t.Errorf("err = %v, want ErrNotConfigured", err)
